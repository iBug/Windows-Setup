@ECHO OFF
PUSHD %~DP0
title Windows 7 Automated Install
set /p DISK_NUM=输入本地硬盘的编号(通常为 0)：

CLS
echo.
title Windows 7 [1/4] 正在准备 DiskPart 命令
...
:STEP2
title Windows 7 [2/4] 正在处理 DiskPart 分区
...
:STEP3
title Windows 7 [3/4] 正在复制 Install.wim 文件
echo [3/4] 正在复制 Install.wim 文件
MKDIR M:\Recovery\WindowsRE > NUL
XCopy.exe Install_7.wim M:\Recovery\WindowsRE\ /Q /H /R /Y /J > NUL
RENAME M:\Recovery\WindowsRE\Install_7.wim Install.wim > NUL
XCopy.exe Unattend_7.xml X:\ /Q /H /R /Y > NUL
RENAME X:\Unattend_7.xml Unattend.xml > NUL
...
echo title Windows 7 [4/4] 正在准备 Install.wim 的安装 >> X:\PostProc.bat
